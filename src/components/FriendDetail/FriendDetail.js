...
const FriendDetail = () => {
    ...
    const handleClick = () => {
        if (details.find(friend => friend.id === parseFloat(friendId))) {
            setOpen(true);
        }
        else {
            setOpen(false);
    ...
    useEffect(() => {
        const foundFriend = details.find(friend => friend.id === parseFloat(friendId));
        setOpen(true);
        setSingleDetails(foundFriend);
    }, [details, friendId]);
                    ...
                    >
                        <Box component="span" sx={{ fontSize: 16, mt: 1, color: '#ffd54f' }}>
                            ...
                            <div style={{ display: 'flex' }}>
                                ...
                                <span style={{ paddingLeft: '10px' }}>{singleDetails?.name}</span>
                            </div>
